...
def tweet(url, tweet_id):
    link = bot(url)
    # time.sleep(10)
    status_text = f"The recent wallet history can be found here: {link}"
    print(status_text)
    client = getClient()
    ...
    for response_line in response.iter_lines():
        ...
        try:
            if response_line:
                ...
                tweeter_address = tweet_text[-42:]
                print(tweet_text)
                print(tweeter_address)
                ...
                print(screen_name)
                # url = 'https://twitter.com/anyuser/status/' + tweet_id
                # print(url)
                tweet_text_case = tweet_text.lower()
                if tweet_text[:2] == 'RT':
                    # print(tweet_text[:2])
                    pass
                if tweet_text_case.startswith('@aifrensbot'):
                    start_thread(tweet, args=[tweeter_address, tweet_id])
                    # link = startBot(url)
                else:
                  pass          
        except:
